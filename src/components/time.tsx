...
import {daysUntilBirthday, USTimeFormatter} from '../utils/constants';
...
function Night({time}: {time: Date}) {
	...
	return (
		<div
			className={clsx('relative flex items-center justify-center overflow-hidden rounded-lg', 'bg-sky-900 text-white')}
		>
			<canvas
				ref={canvasRef}
				aria-hidden
			...
			<div className="z-10 text-center">
				...
				>
					{USTimeFormatter.format(time)}
				</h2>

				<p className={clsx('text-xs font-light', 'text-glow-sky-900 dark:text-glow-sky-500')}><em>in nashville</em></p>
			</div>
		</div>
	);
}
...
function Day({time}: {time: Date}) {
	return (
		<div className="relative flex overflow-hidden rounded-lg bg-gradient-to-br from-[#0A8DFF] to-[#98CFFF] first-letter:w-full">
			<div className="flex w-full items-center justify-center text-center">
				<div className="opacity-85 flex items-baseline space-x-1 md:space-x-2">
					<div className="flex flex-col">
						<div className="font-title text-2xl text-white">{USTimeFormatter.format(time)}</div>
						<p
...
export function Time() {
	...
	return (
		<div className="col-span-3 grid grid-cols-1 gap-6 md:col-span-1 h-full">
			{isNight ? <Night time={time} /> : <Day time={time} />}

			<div className="flex items-center justify-center rounded-lg bg-indigo-100 text-indigo-500 dark:bg-[#23224c] dark:text-indigo-400">
				<div className="text-center">
					<p className="text-xs font-light">
						<span className="font-title text-xl">{daysUntilBirthday}</span> <em>days
						<br />
						until birthday</em>
					</p>
				</div>
